...
router.get("/tags", controller.getProductsByTags);
router.get("/sizes", controller.getAllSizes);
router.get("/colors", controller.getAllColors);
router.get("/collection/:collectionId", controller.getProductsByCollection);
router.get("/sku/:sku", controller.getProductBySKU);
router.get("/low-stock", validateToken, verifyRoles(["admin", "manager"]), controller.getLowStockProducts);
