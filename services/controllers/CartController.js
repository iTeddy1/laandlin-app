...
const getCart = async (req, res) => {
  ...
  try {
    ...
    let availableCart = await Cart.findById(id);
    ...
    availableCart.items.forEach((item) => {
      if (mappedItems[item.productId]) {
        let newColor = mappedItems[item.productId].colors.find((color) => color.color == item.color);
        let newSize = mappedItems[item.productId].sizes.find((size) => size.size == item.size) || "";
    ...
    return res.status(200).json({ cart: items, subTotal: subTotal });
  } catch (err) {
...
const addItemToCart = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: "Item added to cart", cart: items, subTotal: subTotal });
  } catch (err) {
    return res.status(500).json({ message: "Internal server error" });
  }
};
...
const updateItemInCart = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: "Item updated in cart", cart: items, subTotal: subTotal });
  } catch (err) {
    return res.status(500).json({ message: "Internal server error" });
  }
};
...
const removeItemFromCart = async (req, res) => {
  ...
  try {
    ...
    cart.updatedAt = Date.now();
    const updatedCart = await cart.save();
    let itemIds = updatedCart.items.map((item) => item.productId);
    let availableItems = await Product.find(
      { _id: { $in: itemIds } },
      { _id: 1, name: 1, price: 1, salePrice: 1, colors: 1, sizes: 1, slug: 1 },
    );
    let mappedItems = {};
    ...
    return res.status(200).json({ message: "Item removed from cart", cart: items, subTotal: subTotal });
  } catch (err) {
    return res.status(500).json({ message: "Internal server error" });
  }
};
