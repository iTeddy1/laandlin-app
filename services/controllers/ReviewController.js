...
const getAllVerifiedReviews = async (req, res) => {
   try {
      ...
      res.status(200).json({ message: 'Verified reviews retrieved successfully', reviews: reviews });
   } catch (err) {
      res.status(500).json({ message: err.message });
   }
};
...
const getAllUnverifiedReviews = async (req, res) => {
   try {
      ...
      res.status(200).json({ message: 'Unverified reviews retrieved successfully', reviews: reviews });
   } catch (err) {
      res.status(500).json({ message: err.message });
   }
};
...
const updateReview = async (req, res) => {
   try {
      ...
      const updatedReview = await Review
         .findByIdAndUpdate(id, { userId, productId, rating, comment, verified, title }, { new: true })
         .populate('user', 'username email phone')
         .populate('product', 'name slug');
...
const getVerifiedReviewsByProductId = async (req, res) => {
   try {
      const reviews = await Review
         .find({ productId: req.params.productId, verified: true })
         .populate('user', 'username email phone')
         .populate('product', 'name slug');
...
const getUnverifiedReviewsByProductId = async (req, res) => {
   try {
      const reviews = await Review
         .find({ productId: req.params.productId, verified: false })
         .populate('user', 'username email phone')
         .populate('product', 'name slug');
