...
const addDiscount = async (req, res) => {
   try {
      ...
      const newDiscount = await Discount.create({ name: name, code: code, discount: discount, startDate: startDate, endDate: endDate });
...
const updateDiscount = async (req, res) => {
   try {
      ...
      const discount = await Discount.findOneAndUpdate({ code: code }, { code: newCode, name: newName, discount: newDiscount, startDate: newStartDate, endDate: newEndDate }, { new: true });
      if (!discount) {
         return res.status(404).json({ message: 'Discount not found' });
      }
      return res.status(200).json({ message: 'Discount updated', discount: discount });
   } catch (err) {
      console.log(err);
      return res.status(500).json({ message: 'Internal server error' });
   }
...
const deleteDiscount = async (req, res) => {
   try {
      const code = req.params.code;
      const discount = await Discount.findOneAndDelete({ code: code });
      if (!discount) {
         return res.status(404).json({ message: 'Discount not found' });
      }
      return res.status(200).json({ message: 'Discount deleted', discount: discount });
   } catch (err) {
      console.log(err);
      return res.status(500).json({ message: 'Internal server error' });
   }
