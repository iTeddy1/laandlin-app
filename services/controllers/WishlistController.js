...
const getWishlist = async (req, res) => {
    ...
    try {
        let availableWishlist = await Wishlist.findById(id);
        const availableItems = await Product.find({ _id: { $in: availableWishlist.items } }, { _id: 1, name: 1, price: 1, salePrice: 1, colors : 1, sizes: 1, slug: 1 });
...
const removeItemFromWishlist = async (req, res) => {
    ...
    try {
        ...
        await wishlist.save();
        const availableItems = await Product.find({ _id: { $in: wishlist.items } }, { _id: 1, name: 1, price: 1, salePrice: 1, colors : 1, sizes: 1, slug: 1 });
