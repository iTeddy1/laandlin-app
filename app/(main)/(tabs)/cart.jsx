        ...
        {cartItems?.length === 0 ? (
          ...
          <>
              ...
              <View className="py-4 mb-20 space-y-3">
                ...
                <View className="h-fit">
                  <View className="flex flex-row justify-between px-4">
                    <View>
                      <Text className="mb-2 text-sm font-bold">Your friend's shopping cart</Text>
                    </View>
                    <Text className="">{totalItems} Products</Text>
                  </View>

                  {/* carts */}
                  <View className="divide-y">
                    {cartItems &&
                      cartItems.map((item, index) => <CartItem item={item} key={item.itemID} />)}
                  </View>
                </View>
            ...
            <View className="fixed bottom-0 left-0 right-0 z-10 flex flex-row items-center justify-between px-3 py-3 bg-white border-t border-gray-300 shadow-3xl lg:hidden">
              <View>
                <Text className="font-light">Subtotal</Text>
